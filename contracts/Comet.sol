...
/**
Base notes:
1. 'governor' is the admin of the protocol. 
2. Pause can only be triggered by pauseGuardian. 
3. BaseToken of the protocol cannot be changed. 
4. supplyKink = separates the low interest rate slope and the high interest rate slope
 */
...
contract Comet is CometMainInterface {
    ...
    // The Commet contract is a big contract and needs many features, but lack of contract size limit it
    // can't store all functions in this contract, that is where extensionDelegate comes. This variable
    // holds the address of CometExt.sol contract address. Whenver a function is called, which is not
    // present in this contract, the fallback() hits. The fallback() then delegatecalls the CommetExt contract
    // and use its code.
    ...
    /// @dev uint104
    // Protocol's supply volume = totalSupplyBase
    // Protocol's borrow volume = totalBorrowBase
    // Rewards only accumulate when protocol's supply/borrow volume is greater than or equal to baseMinForRewards
    uint public immutable override baseMinForRewards;

    /// @notice The minimum base amount required to initiate a borrow
    // If I borrow something then I must borrow that amount or more than that. I can't keep a
    // borrow position of less than that amount
    ...
    function getAssetInfo(
        ...
        // numAssets = uint8(config.assetConfigs.length) => Is set in constructor
    ...
    function getNowInternal() internal view virtual returns (uint40) {
        // 2**40 = 1099511627776 
        // type(uint40).max = 1099511627775 
        // So here we can see if block.timestamp is equal or greater than 2**40 then it will 
        // not fit in uint40. 
    ...
    function accruedInterestIndices(
        ...
        // At first baseSupplyIndex = 1e15, baseBorrowIndex = 1e15
        uint64 baseSupplyIndex_ = baseSupplyIndex;  
        uint64 baseBorrowIndex_ = baseBorrowIndex;

        // If even a seconds has passed since the last accrueInternal() call then - 
        if (timeElapsed > 0) {
    ...
    function accrueInternal() internal {
        // getNowInternal() => block.timestamp 
    ...
    /**
    supplyRate = how much interest lenders earn per second. 
    Ex- Annual interest rate is 5%, so per second rate = 5% / 365 days = 5% / 31,536,000

    supplyPerSecondInterestRateBase = This is the floor interest rate lenders always get even
    when utilization is 0. Means they will earn interest even when there is no borrow. 

    supplyPerSecondInterestRateSlopeLow = how much the interest rate increases when utilization rate is below/equal supplyKink.
    supplyPerSecondInterestRateSlopeHigh = how much the interest rate increases when utilization rate is above supplyKink.

    Let see how we will read the supplyRate. Suppose the supplyRate is: 796415362. 
    So, supplyRate is a per-second rate scaled by 1e18 i.e multiplied by 1e18. 
    Step-1: Lets get the actual value by dividing it by 1e18. 
    796415362 / 1e18 = 0.000000796415362 
    Step-2: Multiply it by 31536000 (seconds in a year). [if we want monthly rate then we can multiply it by 30 days == 2592000 seconds instead of 31536000]
    0.000000796415362 * 31536000 = 0.02511886415362 
    Step-3: Multiply it by 100 to get the percentage. 
    0.02511886415362 * 100 = 2.511886415362% 
    So, the annual interest rate i.e APY is 2.511886415362%. 
     */
    ...
    function getSupplyRate(
        ...
        /**
        supplyKink => In simple word if the protocol is used lightly or heavily. 
        supplyKink and utilization rate are in same scale.  
         */

        /*
        when utilizationRate is below/equal to supplyKink then the interest rate will increase by (slopeLow * utilization). 
        When utilizationRate is above supplyKink then interest rate will increase by [(slopeLow * supplyKink) + {(slopeHigh * (utilization - supplyKInk)}]. Where (slopeLow * supplyKink) = the increase till supplyKink limit and {(slopeHigh * (utilization - supplyKInk)} = the increase of current utilization to supplyKink.
         */
    ...
    // Check both in etherscan
    // USDC market (baseToken = USDC) => 0xc3d688B66703497DAA19211EEdff47f25384cdc3
    // WETH market (baseToken = WETH) =>  0xA17581A9E3356d9A858b789D68B4d866e593aE94
    // borrowKink = 900000000000000000 = 0.9 * 1e18 = 90%
    // supplyKink = 900000000000000000 = 0.9 * 1e18 = 90%
    // supplyPerSecondInterestRateSlopeHigh = 101344495180 
    // supplyPerSecondInterestRateSlopeLow = 1141552511
    // borrowPerSecondInterestRateSlopeHigh = 114155251141 
    // borrowPerSecondInterestRateSlopeLow = 880834601

    /**
    utilizationRate < supplyKink :
        Means there are enough baseToken present in the contract. Lenders earns less 
        interest because as there are less borrower so less interst coming in so small
        interest gets split among lenders. In this case pool is healthy because Lenders can
        withdraw their deposit (as pool has enough baseToken)
    utilizationRate > supplyKink :
        Means there are less baseToken present in the contract. Lenders earns more 
        interest because as there are more borrower so more interst coming in so large
        interest gets split among lenders. In this case pool is unhealthy because Lenders
        cannot withdraw their deposit as much they wish (as pool has less baseToken).

    utilizationRate < borrowKink: 
        means there are more baseToken to borrow. As the pool has enough baseToken so - 
        (i) borrowRate is lower, so that more borrower gets attracted
        (ii) As there are not enough borrowing so lenders earn less interest. 
        (iii) Pool is healthy because lenders can withdraw their deposit (as pool has enough baseToken)
    utilizationRate > borrowKink:
        means there are less baseToken to borrow. As the pool has not enough baseToken so - 
        (i) borrowRate is higher, so that less borrower gets attracted and liquidity stay protected. 
        (ii) As there are heavy borrowing so lenders earn more interest. 
        (iii) Pool is unhealthy because lenders cannot withdraw their deposit (as pool has not enough baseToken)
    ...
    function getUtilization() public view override returns (uint) {
        /**
        totalSupplyBase  =  sum of all original deposits of baseToken (principal only, no interest)
        totalSupply()    =  totalSupplyBase + all accrued interest
         */
        ...
        /**
        totalBorrowBase  =  sum of all original borrows of baseToken (principal only, no interest)
        totalBorrow_     =  totalBorrowBase + all accrued borrow interest
         */
        ...
        if (totalSupply_ == 0) {
            ...
            /**
             This is how FACTOR_SCALE is used:
             totalBorrow = 800 usdc = 800_000_000
             totalSupply = 1000 usdc = 1_000_000_000
             FACTOR_SCALE = 1e18 = 1_000_000_000_000_000_000
             (800_000_000 * 1_000_000_000_000_000_000) / 1_000_000_000 
             = 800_000_000_000_000_000 = 0.8 * 1e18 = 80% utilization
             */
    ...
    function doTransferIn(
        ...
        // @doubt: why Non-standard erc20 interface is used ?
        ...
        assembly ("memory-safe") {
            ...
            // If returndatasize 0 byte it means the token transfer function returns nothing. Tokens like 
            // USDT returns nothing from transfer/transferFrom. 
            // not(0) = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF
            // when solidity reads 'success' any non-zero value is true
            case 0 {   
                // This is a non-standard ERC-20
                success := not(0) // set success to true
            }
            // If returndatasize is 32 bytes then copy the 32 bytes value into memory location 0. 
            // then load the value to 'success', here the value could be true/false
            case 32 {
            ...
            // If returndatasize is anything else, it's an excessively non-compliant ERC-20, revert.
            default {
        ...
        return 
            // This function returns the actual received amount. Because a token could be fee-on-transfer 
            // token, if so then this contract will receive less token than actually sent. 
            IERC20NonStandard(asset).balanceOf(address(this)) -
            preTransferBalance;
    }
    ...
    function supplyInternal(
        ...
        // If supplied asset is base token then it means we are acting as lender
        if (asset == baseToken) {
     ...
     * @dev Supply an amount of base asset from `from` to dst 
     Supplying base : from = msg.sender, dst = msg.sender
     */
    function supplyBase(address from, address dst, uint256 amount) internal { 

        // amount = supplied token amount to this contract, if the token is fee-on-transfer then 
        // this amount will be lesser than the actual amount sent. 
